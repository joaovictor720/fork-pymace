...
gcounter<int, std::string> _pending_delta;
...
void delta_flush_thread(stats &st, double interval) {
    while (true) {
        std::this_thread::sleep_for(std::chrono::duration<double>(interval));

        gcounter<int, std::string> delta_to_send;
        {
            std::unique_lock<std::mutex> lg(_pending_mutex);
            if (!_has_pending) {
                continue;
            }
            delta_to_send = _pending_delta;
            _pending_delta = gcounter<int, std::string>();
            _has_pending = false;
        }

        std::string ser = delta_to_send.serialize();
...
int main(int argc, char* argv[]) {
    ...
    t_hb.detach();
    std::thread t_app_sender(delta_flush_thread, std::ref(st), nc.dissemination_interval);
    t_app_sender.detach();
