...
public:
  ...
  // Serializa o estado atual (delta) para um buffer de bytes
  std::string serialize() const {
      std::ostringstream oss;
      
      // Quantos itens temos no mapa? (Útil se enviar o mapa inteiro, 
      // mas para Deltas simples, apenas iteramos)
      size_t mapSize = m.size(); 
      oss.write(reinterpret_cast<const char*>(&mapSize), sizeof(mapSize));

      for (const auto& kv : m) {
          // 1. Tamanho da String ID
          size_t keyLen = kv.first.size();
          oss.write(reinterpret_cast<const char*>(&keyLen), sizeof(keyLen));
          
          // 2. A String ID em si
          oss.write(kv.first.data(), keyLen);
          
          // 3. O Valor do Contador
          oss.write(reinterpret_cast<const char*>(&kv.second), sizeof(kv.second));
      }
      return oss.str();
  }

  // Reconstrói um gcounter a partir de bytes recebidos
  static gcounter<V,K> deserialize(const std::string& data) {
      gcounter<V,K> temp; // Cria um counter vazio (sem ID, pois só serve para merge)
      std::istringstream iss(data);
      
      size_t mapSize = 0;
      if (iss.read(reinterpret_cast<char*>(&mapSize), sizeof(mapSize))) {
          for (size_t i = 0; i < mapSize; ++i) {
              // 1. Ler tamanho da chave
              size_t keyLen = 0;
              iss.read(reinterpret_cast<char*>(&keyLen), sizeof(keyLen));
              
              // 2. Ler a chave
              std::string key(keyLen, '\0');
              iss.read(&key[0], keyLen);
              
              // 3. Ler o valor
              V val;
              iss.read(reinterpret_cast<char*>(&val), sizeof(val));
              
              // Inserir no mapa temporário
              temp.m[key] = val;
          }
      }
      return temp;
