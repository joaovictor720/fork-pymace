...
CATALOG_DIR = ROOT_DIR / "evaluation" / "trace_catalogs" / "spatial_grid_1km_20x20"
...
class SpatialTraceCatalogTests(unittest.TestCase):
    def test_catalog_shape_and_first_t_cover_range(self):
        ...
        self.assertEqual(catalog["grid"]["cell_count"], 400)
        ...
        for run in catalog["runs"]:
            ...
            self.assertEqual(manifest["grid"]["rows"], 20)
            self.assertEqual(manifest["grid"]["cols"], 20)
            ...
            for prefix in ("10", "20", "30", "40", "50"):
                ...
                self.assertEqual(validation["covered_cell_count"], 400)
                self.assertEqual(validation["total_cell_count"], 400)
